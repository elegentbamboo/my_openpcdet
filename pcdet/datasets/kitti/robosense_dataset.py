...
class RobosenseDataset(DatasetTemplate):
    ...
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        # 调用函数，加载robosense数据，mode的值为：train 或者  test
    ...
    def get_files_name_list(self):
        ...
        for per_folder in folder_list:
            ...
            # 目标物的文件夹的路径myobject_path
            myobject_path = str(self.root_path + per_folder + '/')
            # 目标物文件夹下的文件列表 myobject_list =['label','pcd']
            myobject_list = self.get_folder_list(myobject_path)

            for one_folder in myobject_list:
                if one_folder == 'pcd':
                    pcd_path = str(myobject_path + one_folder)
                if one_folder == 'label':
                    label_path = str(myobject_path + one_folder)
    ...
    def get_all_labels(self, num_workers=4, files_list_label=None):
        ...
        # 信息包括：type, position ,scale,rotation,id等信息
        global i
        i = 0
        ...
        def get_single_label_info(single_label_path):
            ...
            #print(single_objects_label_info['single_label_path'])
            #single_objects_label_info_car = np.array([label['obj_type']['Car'] for label in labels])
            #single_objects_label_info_Pedestrian = np.array([label['obj_type']['Pedestrian'] for label in labels])
            single_objects_label_info['name'] = np.array([label['obj_type'] for label in labels])
            # for label in labels :
            #     obj_type={}
            #     obj_type['0']=label['obj_type']['Car']
            #     obj_type['1'] = label['obj_type']['Pedestrian']
            #     single_objects_label_info['name'] = {'name':obj_type}

            #     vfsingle_objects_label_info_type = {}
            #     single_objects_label_info_type['obj_type']['0']=label['obj_type']['Car']
            #     single_objects_label_info_type['obj_type']['1'] = label['obj_type']['Pedestrian']


                #print(single_objects_label_info['name'])
            single_objects_label_info['box_position'] = np.array(
                [[label['psr']['position']['x'], label['psr']['position']['y'], label['psr']['position']['z']] for label in labels])
            single_objects_label_info['box_scale'] = np.array(
                [[label['psr']['scale']['x'], label['psr']['scale']['y'], label['psr']['scale']['z']] for label in labels])
            single_objects_label_info['box_rotation'] = np.array(
                [[label['psr']['rotation']['x'], label['psr']['rotation']['y'], label['psr']['rotation']['z']] for label in
                 labels])
            single_objects_label_info['obj_id'] = np.array([label['obj_id'] for label in labels])

            box_position = single_objects_label_info['box_position']
            box_scale = single_objects_label_info['box_scale']
            box_rotation = single_objects_label_info['box_rotation']

            rotation_yaw = box_rotation[:, 2].reshape(-1, 1)
            gt_boxes = np.concatenate([box_position, box_scale, rotation_yaw], axis=1).astype(np.float32)
    ...
    def __getitem__(self, index):
        ...
        #####提取文件名######
        single_pcd_path_filename = os.path.basename(single_pcd_path)
        single_pcd_path_filename = single_pcd_path_filename.split('.')[0]
        # 定义输入数据的字典，包含：points，文件的路径，。。？
        input_dict = {
            'points': points,
            'frame_id': float(single_pcd_path_filename),
            'single_pcd_path': float(single_pcd_path_filename),
        }
        ...
        box_position = single_objects_label_info['box_position']  # (N,3)
        box_scale = single_objects_label_info['box_scale']  # (N,3)
        box_rotation = single_objects_label_info['box_rotation']  # (N,3)
        obj_id = single_objects_label_info['obj_id'].astype(np.float32) # (N,)

        # 以下是将 上面的3D框的数据 转化为统一的数据格式
        # 数据格式为：(N,7)，分别代表 (N, 7) [x, y, z, l, h, w, r]
        # gt_boxes: [x, y, z, dx, dy, dz, heading], (x, y, z) is the box position"""
        rotation_yaw = box_rotation[:, 2].reshape(-1, 1)
        gt_boxes = np.concatenate([box_position, box_scale, rotation_yaw], axis=1).astype(np.float32)
    ...
    def create_groundtruth_database(self, info_path=None, used_classes=None, split='train'):
        ...
        for k in range(len(infos)):
            ...
            name = info['name']
            box_position = info['box_position']
            box_scale = info['box_scale']
            ...
            for i in range(num_obj):
                ...
                if (used_classes is None) or name[i] in used_classes:
                    ...
                    db_info = {
                        ...
                        'box_position': box_position, 'box_scale': box_scale, 'box_rotation': box_rotation,
                        'obj_id': obj_id
                    }
    ...
    def generate_prediction_dicts(batch_dict, pred_dicts, class_names, output_path=None):
        ...
        def get_template_prediction(num_samples):
            ret_dict = {
                ...
                'box_position': np.zeros([num_samples, 3]),
                'box_scale': np.zeros([num_samples, 3]),
                'box_rotation': np.zeros([num_samples, 3]),
                'obj_id': np.zeros(num_samples),
        ...
        def generate_single_sample_dict(box_dict):
            ...
            pred_dict['box_position'] = pred_boxes[:, 0:3]
            pred_dict['box_scale'] = pred_boxes[:, 3:6]
        ...
        for index, box_dict in enumerate(pred_dicts):
            ...
            if output_path is not None:
                ...
                with open(cur_det_file, 'w') as f:
                    ...
                    box_position = single_pred_dict['box_position']
                    box_scale = single_pred_dict['box_scale']
                    box_rotation = single_pred_dict['box_rotation']

                    for idx in range(len(single_pred_dict['name'])):
                        print('%s,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,'
                              % (name[idx],
                                 box_position[idx][0], box_position[idx][1], box_position[idx][2],
                                 box_scale[idx][0], box_scale[idx][1], box_scale[idx][2],
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        ...
        # name , box_position,box_scale,box_rotation,tracked_id,    scores,pred_labels,pred_lidar,frame_id
        '''
        print('~~~~~~~~~~~~~det_annos~~~~~~~~~~~~~~~~~~')
        print(det_annos[0])
        print(len(det_annos))
        ...
        from ..kitti.kitti_object_eval_python import myeval as kitti_eval
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_robosense_infos':
        ...
        class_names = [ 'pedestrain', 'car']

        create_robosense_infos(
            dataset_cfg=dataset_cfg,
            class_names=class_names,
            data_path='/Project/pycharm_project/OpenPCDet-master/data/robosense/',
            save_path='/Project/pycharm_project/OpenPCDet-master/data/robosense/'
        )
